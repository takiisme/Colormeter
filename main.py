...
df = corrector_scaling.apply_correction(df, prefix="scaling_correction")
df = convert_rgb_cols(df, prefix="scaling_correction_r4_", to=ColorSpace.LAB)
...
eucl_raw_rgb = np.sqrt(
    (df["color_r4_R"] - df["gt__R"])**2 + \
    (df["color_r4_G"] - df["gt__G"])**2 + \
    (df["color_r4_B"] - df["gt__B"])**2
)
axs[0] = plot_ecdf(axs[0], eucl_raw_rgb, label="raw", color='blue')
eucl_scaling_rgb = np.sqrt(
    (df["scaling_correction_r4_R"] - df["gt__R"])**2 + \
    (df["scaling_correction_r4_G"] - df["gt__G"])**2 + \
    (df["scaling_correction_r4_B"] - df["gt__B"])**2
)
axs[0] = plot_ecdf(axs[0], eucl_scaling_rgb, label="scaling in RGB", color='red')
eucl_raw_lab = np.sqrt(
    (df["color_r4_l"] - df["gt__l"])**2 + \
    (df["color_r4_a"] - df["gt__a"])**2 + \
    (df["color_r4_b"] - df["gt__b"])**2
)
axs[1] = plot_ecdf(axs[1], eucl_raw_lab, label="raw", color='blue')
eucl_scaling_lab = np.sqrt(
    (df["scaling_correction_r4_l"] - df["gt__l"])**2 + \
    (df["scaling_correction_r4_a"] - df["gt__a"])**2 + \
    (df["scaling_correction_r4_b"] - df["gt__b"])**2
)
axs[1] = plot_ecdf(axs[1], eucl_scaling_lab, label="scaling in RGB", color='red')

# Model
df_train = df_raw.sample(frac=0.8, random_state=0)
