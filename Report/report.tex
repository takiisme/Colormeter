...
The smallest wavelength difference that can be perceived as a chromatic difference under constant luminance conditions is approximately $1$~nm, which corresponds to about an Euclidean distance of approximately $1.27$ in the $L^* a^* b^*$ color space \citep{oleari2015}.
The authors further mentioned that a few multiples of the just noticeable difference can still be considered small.
Accordingly, we define the Euclidean error $6$ (approximately five times the jost noticable difference) as the \emph{small color difference threshold} in our study.
...
In median, correction by scaling reduces the Euclidean error by $55\%$ relative to uncorrected colors,
...
We find that color 11 (\textcolor{c11}{yellow green}) is the primary driver of failure of generalization.
...
although it does not prevail over the scaling method in the average case.
