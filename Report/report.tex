...
We statistically analyze a deterministic and a data-driven color correction method which improves the accuracy of color detection using smartphone images,
...
To evaluate the performance of the correction methods, we define an accuracy thresholds.
The first corresponds to the human chromatic sensitivity threshold, while the second represents a small but perceptible color difference.
\todo{Delete this sentence!}
The smallest wavelength difference that can be perceived as a chromatic difference under constant luminance conditions (''just noticeable difference'') is approximately $1$~nm, which corresponds to about an Euclidean distance of approximately $1.27$ in the $L^* a^* b^*$ color space \citep{oleari2015}.
The authors further mentioned that a few multiples of the just noticeable difference can still be considered small.
Accordingly, we define the Euclidean error $6$ (approximately five times the just noticeable difference) as the \emph{small color difference threshold} in our study.
...
Along the $b^*$ axis, the measured values are $-3.53 \pm 2.07$, whereas the D65 standard white has a value of $-0.0104$, indicating a slight tendency toward blue-ish color shifts.
...
\begin{equation*}
    ...
    \end{pmatrix}.
\end{equation*}
% This method was suggested by \citet{degreef2014} for monitoring newborn jaundice using smartphones, but not evaluated for general-purpose color detection.
...
We apply scaling correction to the same test set.\todo{
...
In median,\todo{
    The median error is ...
} correction by scaling reduces the Euclidean error by $55\%$ relative to uncorrected colors,
while the full and reduced models yield a reduction of $65\%$ and $66\%$, respectively.
Without correction, only $2\%$ of measured colors fall below the small color difference threshold (Euclidean error $\leq 6$).
Using correction by scaling, we increase the proportion to $26\%$.
...
\todo{Bootstrap CI table.}
The statistics reported above also show no improvement of the full model over the reduced one.
Thus, we focus on the reduced model in subsequent analysis.

\begin{table*}
    \caption{%
        Coefficient estimates on training set with $95\%$ percentile bootstrap confidence intervals \cite{davison1997,efron1994}.
        To construct the confidence intervals, we resample training instances with replacement, which corresponds to ``resampling cases'' in \citet{davison1997} and ``bootstrap pairs'' in \citet{efron1994}.
        We run 1000 bootstrap trials to obtain the confidence intervals.
    }
    \label{tab:boot}
    \input{../boot_table.tex}
\end{table*}
...
whereas the error distributions in the other two channels are all centered at approximately zero.\todo{
    Models make the error concentrated.
}
...
We find that color 11\todo{
    Explain this color indexing.
} is the primary driver of failure of generalization.
Notably, color 11 is both a member of the Greenish\todo{
    Avoid this term.
} group and possesses the largest green ($G$) channel intensity in the dataset.
...
although it does not prevail over the scaling method in the average case.\todo{
...
Zhi Jing performed exploratory analysis, implemented the models, and conducted preliminary experiments with them.
...
T\`ai Th\'ai performed exploratory analysis, designed the cross validation experiments, and improved the models.
Jonas Thumbs developed the app for data collection, performed exploratory analysis, and designed the graphical abstract.
Baisu Zhou performed exploratory analysis, participated in implementation and analysis of the methods, and finalized the figures and the report.
