...
\begin{figure*}
    \includegraphics{../Images/graphical_abstract.pdf}\\
    \caption{
...
After correction by the full and reduced models, the median errors drop to $7.22$ and $7.17$, respectively.
Without correction, only $2\%$ of measured colors fall below the small color difference threshold (Euclidean error $\leq 6$).
Using correction by scaling, the small color difference rate is increased to $24\%$.
The full and reduced models yield a small color difference rate of $38\%$ and $36\%$, respectively.
...
The observation is also supported by \cref{tab:boot}, which shows the pitch and roll coefficient estimates of the full model fitted on the training set along with their $95\%$ bootstrap confidence intervals.
Both pitch and roll have non-significant effects on color correction.
Based on these results, we focus on the reduced model in subsequent analysis.
