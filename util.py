...
def get_color_col_names(df: pd.DataFrame, r: int, space: ColorSpace, correction: bool = False, gt: bool = False):
    """
    Check if required color columns exist in the DataFrame and return their names.
    """
    channels = space.get_channels()
    cols = [f'color_r{r}_{ch}' for ch in channels]
    if correction:
        cols += [f'correction_r{r}_{ch}' for ch in channels]
    if gt:
        cols += [f'gt__{ch}' for ch in channels]
    for col in cols:
        assert col in df.columns, f"DataFrame missing required column: {col}"
    return cols
